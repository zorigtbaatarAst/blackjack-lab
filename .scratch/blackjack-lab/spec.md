...
- **Stable layout:** nothing on screen moves between turns. The betting panel (bet line, chip tray, Clear/Rebet/Deal) and the action panel are stacked in one grid cell, and only one is visible at a time: no chips during a Round. The cell keeps the taller panel's height, and hidden panels' buttons are disabled so keys can't reach them. The Actions are one row of normal-height buttons in the same spot as Clear/Rebet/Deal. The player preferred it to larger 2×2 buttons. Train stacks its Actions and Next the same way. Coach Mistakes, the Hint and the Round's result appear in a fixed-height slot printed on the felt, not in a panel that grows and shrinks.
- **Auto bet:** a session-only toggle (key A). After each Round's reveal it re-deals the current Bet 1.5 s later, so the player only makes Decisions. While Auto is on, the table never drops back to the betting panel: the greyed Action row stays through the pause. To change the Bet, turn Auto off. Auto won't start without a Bet it can deal. Turning it on at an idle table deals right away. It stops, with a notice, if Settlement had to lower the Bet because the last one no longer fits the Bankroll. It pauses while another tab or a dialog is open, and resumes on returning to Play.
- **Desktop:** cards size to the smaller of window width and height (46–112 px); from 900 px wide the column widens to 760 px. Keyboard: H/S/D/P for the Actions and drill answers, 1–4 for chips, Enter to deal or go to the next Situation, C to clear, R to rebet, Esc to close a dialog. Keys press the same buttons a tap would. Key hints (the letters on buttons and the keys line) are help, so they show only while the Hint is on, and only on devices with a mouse. The keys work regardless. Hover states show on mouse devices.
...
- **Improve screen:** accuracy (overall and by category), recent Mistakes, Play stats, the Strategy chart (28×10), the leaderboard section, and Reset stats with a confirmation. Chart cells:
  - Each cell is filled with its Book action's colour (Hit green, Stand magenta, Double yellow, Split blue). Ds cells are split diagonally, Double / Stand. The Action buttons keep the classic table colours (the player's preference: Stand red), which is fine because they carry their names.
